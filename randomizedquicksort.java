...
public class RandomizedQuickSort {
    public static void randomizedQuickSort(int[] arr, int low, int high, Random rnd) {
        if (low < high) {
            int k = rnd.nextInt(high - low + 1) + low;
            int temp = arr[k];
            arr[k] = arr[high];
            arr[high] = temp;
            int p = partition(arr, low, high);
            randomizedQuickSort(arr, low, p - 1, rnd);
            randomizedQuickSort(arr, p + 1, high, rnd);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.printf("=== n=%d — RandomizedQuickSort — %d ejecuciones ===%n", N, REPS);
        for (int r = 0; r < REPS; r++) {
            int[] arr = base.clone();
            long t0 = System.nanoTime();
            randomizedQuickSort(arr, 0, arr.length - 1, rnd);
        ...
        System.out.printf("%nPROMEDIO (%d) RandomizedQuickSort n=%d: %.6f s%n", REPS, N, promedio);
    }
}
